    ...
    window.jQuery.ajax({
            ...
            window.jQuery.get('https://raw.githubusercontent.com/narutocraft3345/Agar.io-bot/master/launcher.user.js?' + Math.floor((Math.random() * 1000000) + 1), function(data) {
                ...
                if (latestVersion > myVersion) {
                    update("aposLauncher", "launcher.user.js", "https://github.com/narutocraft3345/Agar.io-bot/blob/" + sha + "/launcher.user.js/");
                }
...
window.refreshTwitch = function() {
    $.ajax({
        url: "https://api.twitch.tv/kraken/streams/narutocraft3345",
        cache: false,
        dataType: "jsonp"
    }).done(function(data) {
        if (data["stream"] == null) {
            //console.log("narutocraft3345 is not online!");
            window.setMessage([]);
            window.onmouseup = function() {};
            window.ignoreStream = false;
        } else {
            //console.log("narutocraft3345 is online!");
            if (!window.ignoreStream) {
                window.setMessage(["twitch.tv/narutocraft3345 is online right now!", "Click the screen to open the stream!", "Press E to ignore."]);
                window.onmouseup = function() {
                    window.open("http://www.twitch.tv/narutocraft3345");
                };
            }
        }
    }).fail(function() {});
